...
if len(sys.argv) != 2:
    print('Usage:\n\tdockerpull.py [registry/][repository/]image[:tag|@digest]\n')
    exit(1)
...
resp = session.get('https://{}/v2/'.format(registry), verify=False)
...
resp = session.get('https://{}/v2/{}/manifests/{}'.format(registry, repository, tag), headers=auth_head, verify=False)
...
for layer in layers:
    ...
    if (bresp.status_code != 200):  # When the layer is located at a custom URL
        bresp = session.get(layer['urls'][0], headers=auth_head, stream=True, verify=False)
