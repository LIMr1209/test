...
class AsyncPool(object):
    ...
    def submit_async(self, func, *args, callback=None):
        """
        非阻塞模式
        提交任务到事件循环
    ...
    def submit_no_async(self, func, *args, callback=None):
        """
        阻塞模式
        提交任务到事件循环

        :param func: 同步函数对象 (没有async 修饰的函数)
        :param args: 参数
        :param callback: 回调函数
        :return:
        """
...
def demo():
    # 任务组， 最大协程数
    pool = AsyncPool(maxsize=50, pool_maxsize=100)

    # 插入任务任务
    for i in range(100 + 1):
        # 非阻塞协程
        pool.submit_async(thread_example2, i, callback=my_callback)
        # 阻塞协程
        # pool.submit_no_async(thread_example4, i, callback=my_callback)
