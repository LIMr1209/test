...
sd=pd.read_table('{}.csv'.format(tag),sep=',',names=['price','sale_count'])
...
for i in range(price_range_len):
    if i == 0:
        ...
        price_range.append([int(abnormal_price / price_range_len*i)+1, int(abnormal_price / price_range_len*(i+1))])
